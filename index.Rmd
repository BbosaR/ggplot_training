...
For more on these visit (https://exts.ggplot2.tidyverse.org/gallery/)[https://exts.ggplot2.tidyverse.org/gallery/]

---

## `autoplotly` (Terry Tang Yuan)

* Automatic generation of interactive visualizations for popular statistical results

* It gives a `plotly` version of the `autoplot()` version of the `ggforge` package.
...
autoplotly(prcomp(mtcars[c(5, 6, 7)]), data = mtcars,
  colour = 'gear', frame = TRUE)
```
...
## GGAlly (Barret Schloerke et al.)

* Extensions of ggplot to reduce geom combinations

.pull-left[
```{r, eval=FALSE, echo=TRUE}
library(GGally)
d1 <- read.csv("Data/data2.csv", header = T)
d1 %>% 
  dplyr::select(Thermal.C, Thermal.D , S.H , Temp  ,M.C) %>%
...
* ggplot version of `pairs()` in base **R**.
...
* The aim is to make publication ready plots with minimal codes.

.pull-left[

```{r, eval=FALSE, echo=TRUE, warning=FALSE}
library(ggpubr)
d2 <- read.table("Data/Asejire_WC.txt", header = T)
d2 <- d2 %>% mutate(Year = factor(Year))
comps <- list(c('2003', '2004'), c('2003', '2005'), 
              c('2003', '2006'), c('2003', '2007'))
ggboxplot(d2, x = "Year", y = "PH",
                color = "Year", palette = 'Dark2',
                add = "jitter", shape = "Year") + 
  stat_compare_means(comparisons = comps, method = 't.test') +        # Add pairwise comparisons p-value
  stat_compare_means(label.y = 8.6)                # Add global p-value
```
...
ggboxplot(d2, x = "Year", y = "PH",
                ...
                add = "jitter", shape = "Year") + 
  stat_compare_means(comparisons = comps, method = 't.test') +        # Add pairwise comparisons p-value
  stat_compare_means(label.y = 8.6)         
```
]

---

## ggstatplot (Indrajeet Patil)

* Also for publication ready plots
...
ggstatsplot::ggbetweenstats(
  ...
  x = Year,
  y = Fe,
  title = "Distribution of Fe between 2003-2007"
)
```
]
...
ggstatsplot::ggbetweenstats(
  ...
  x = Year,
  y = Fe,
  title = "Distribution of Fe at Asejire between 2003-2007"
)
```
]

* provides fucntions to annotate bar, pie, linear models and meta-anlysis.
...
library(survival)
library(survminer)
sfit <- survfit(Surv(time, status) ~ voltage, data = capacitor)
ggsurvplot(sfit,
           legend.title = "Voltage",
           palette = 'Dark2', 
           conf.int = TRUE,
           pval = TRUE,
           risk.table = TRUE, 
...
## gganimate (Thomas Lin Pedersen)

* Awesome way to animate your plots. The package really packs a punch.

.pull-left[

```{r, echo=TRUE, eval=FALSE, warning=FALSE}
library(gganimate)
d2 %>% ggplot(aes(Month, PH, group = Year)) + 
  geom_line() + 
  geom_segment(aes(xend = 12, yend = PH), linetype = 2, colour = 'grey') + 
  geom_point(size = 4) + 
  geom_text(aes(x = 12.1, label = Year), hjust = 0) + 
  transition_reveal(Month) + 
  coord_cartesian(clip = 'off') + 
  labs(title = 'Ph of Asejire', y = 'Ph') + 
  theme_minimal() + 
...
## ggpol (Frederik Tiedemann)

.pull-left[

```{r, echo=TRUE, eval=FALSE, warning=FALSE}
library(ggpol)
house_of_rep <- data.frame(Parties = factor(c('APC', 'PDP', 'Others'),
                                            levels = c('APC', 'PDP', 'Others')),
                           Seats = c(204, 118, 15),
                           colors = c('skyblue1', 'red', 'grey82'),
...
house_of_rep %>%
  ...
  scale_fill_manual(values = house_of_rep$colors, 
                    labels = house_of_rep$Parties) +
  coord_fixed() + 
  theme_void()
...
<img src="picture/NNMR_by_region.png" width="800" height="800">
...
<h3> From all of us to you, THANK YOU! `r emo::ji('pray')``r emo::ji('pray')``r emo::ji('pray')`
